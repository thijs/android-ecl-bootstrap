...
(let* ((cmd-args (ext:command-args))
  ...
  (with-open-file (o "../fas/load.lisp" :direction :output :if-exists :supersede)
    ...
    (format o "    (let ((module-location (truename (format nil \"~~a~~a.fas\" dir module))))~%")
    (format o "      (android-log (format nil \"found ~~a: ~~a\" module module-location))~%")
    (format o "      (load module-location)~%")
    (format o "      (android-log (format nil \"loaded ~~a: ~~a\" module module-location)))))~%")))
